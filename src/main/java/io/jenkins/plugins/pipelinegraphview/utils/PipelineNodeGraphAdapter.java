...
public class PipelineNodeGraphAdapter {
  ...
  private Map<String, String> getNodesToRemap(List<FlowNodeWrapper> pipelineNodesList) {
    ...
    for (FlowNodeWrapper node : pipelineNodesList) {
      for (FlowNodeWrapper parent : node.getParents()) {
        ...
        /*
        // 3. If the node has a parent which is a stage, with the same name and has only one child (this node) then remap child nodes to that parent. This removes some superfluous stages.
        if (node.getType() == FlowNodeWrapper.NodeType.STAGE
            && parent.getType() == FlowNodeWrapper.NodeType.STAGE
            && node.getDisplayName() == parent.getDisplayName()
  ...
  private void dumpNodeGraphviz(
      List<FlowNodeWrapper> localPipelineNodesList,
  ...
  private void remapStageParentage() {
    ...
    this.pipelineNodesList =
        this.pipelineNodesList.stream()
            ...
            filter(n -> n.getParents().isEmpty() || n.getParents().get(0).getStatus().wasExecuted())
